"""
================================================================================
                    NEXUS REAL-TIME MULTIMODAL DESKTOP AGENT
...
if (REPOS_DIR / "nexus" / "src").exists():
    sys.path.insert(0, str(REPOS_DIR / "nexus" / "src"))
...
logger = logging.getLogger("NexusRealTime")
...
\033[0m
\033[93m>>> NEXUS REAL-TIME VOICE SYSTEM (100% LOCAL & ZERO-MOCK) <<<\033[0m
================================================================================
...
* Wake Engine: openWakeWord ONNX ('Hey Nexus')
...
class RealTimeNexus:
    """Master controller for Real-time Nexus execution loop."""
    ...
    def start(self):
        ...
        startup_phrase = (
            "Nexus online. All subsystems operational. "
        ...
        try:
            self.wake_detector = NexusWakeDetector(threshold=0.28, on_wake=self._on_wake_triggered)
    ...
    def _on_wake_triggered(self):
        """Callback invoked when 'Hey Nexus' wake word is detected."""
        if self._is_busy:
            return
        ...
        try:
            ...
            if query:
                ...
                print("\033[90m[Nexus: No command heard - standing by for 'Hey Nexus']\033[0m")
        finally:
            time.sleep(0.4)
            if self.wake_detector:
    ...
    def _interactive_console_loop(self):
        ...
        """
        print("\n\033[1;33m[NEXUS READY]\033[0m Options:")
        print("  - Say aloud: \033[1;32m'Hey Nexus'\033[0m (via G435 Headset)")
        ...
        while self._running:
            try:
                user_input = input("\033[93mNEXUS >> \033[0m").strip()
                
                if user_input.lower() in ["exit", "quit", "shutdown"]:
                    self.tts.speak("Shutting down Nexus systems. Goodbye, Sir.")
                    self._running = False
                    break
